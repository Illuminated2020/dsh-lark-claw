...
在飞书应用的“事件与回调 → 回调配置”中选择使用长连接接收回调，订阅新版 `card.action.trigger`（卡片回传交互），并使配置生效。仅订阅 `im.message.receive_v1` 只能收到聊天消息，不能完成卡片回答。更新插件后重启服务。

dsh 操作审批使用独立的“允许本次／拒绝”卡片，展示工具、理由及可用的原始调用参数；指定调用的参数无法找到时不提供批准入口。批准仅适用于该次操作。审批策略为 `never` 时，dsh 仍会直接拒绝，插件不覆盖该策略。

`ask_user_question` 支持单选、多选和补充文字；方案确认展示完整方案及原请求的选项。点击“提交回答”后，结构化答案返回正在等待的工具；点击“取消”、停止任务、服务关闭或超时均结束等待。若提交缺少必答内容或包含无效选项，卡片会提示重新填写。标准 dsh Agent 预设提供提问工具；自定义预设需保留相应工具和服务。

卡片发在执行卡片所在话题，由任务发起人回答，不需要再次 @ 机器人。回调校验连接、聊天、卡片 ID 和用户 ID；重复点击及过期卡片不能重复批准。问题和结果由 dsh 的审批审计或工具结果记录，已处理卡片显示最终状态。若卡片状态更新失败，旧按钮也不会再生效。等待中的请求不跨重启恢复。

`gateway.interaction_timeout_ms` 设置等待上限，默认 `600000`（十分钟）。没有当前用户发起的定时任务不创建可供群成员任意批准的交互卡片；没有其他应答器时，dsh 按自身规则返回审批不可用或提问失败。
